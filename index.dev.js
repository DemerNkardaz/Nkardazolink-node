...
app.get('/:lang?/wiki/:page', async (request, response, next) => {
  ...
  */
  ...
  const blockquote = {
    ...
    Вид с горы [[Хаппон-гёран]] (八本暁嵐) на здешние поля и облака поистине прекрасен.<br>
  ...
  `);/*
    ...
    const articleContent = await new JSDOM(`<!DOCTYPE html><body>${await new WikiMarkup().render(markup)}</body>`).window.document;
    const articleHeadings = articleContent.querySelectorAll('h2');

    if (articleHeadings.length > 0) {
      const nonHeadedWrapper = articleContent.createElement('div');
      nonHeadedWrapper.className = 'nw-article-nonheaded-wrapper';

      let currentElement = articleContent.body.firstChild;
      ...
      articleContent.body.insertBefore(nonHeadedWrapper, articleHeadings[0]);
    }

    articleHeadings.forEach((heading) => {
      const wrapper = articleContent.createElement('div');
    ...
    wikiContent.appendChild(await new WikiBuilder(markup).build);
